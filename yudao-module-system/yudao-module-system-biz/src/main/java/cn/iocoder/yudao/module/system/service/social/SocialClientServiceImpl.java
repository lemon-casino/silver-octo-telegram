...
import cn.iocoder.yudao.module.system.auth.AuthDingTalkRequest;
...
public class SocialClientServiceImpl implements SocialClientService {
    ...
    public AuthUser getAuthUser(Integer socialType, Integer userType, String code, String state) {
        ...
        AuthRequest authRequest = buildAuthRequest(socialType, userType);
        AuthCallback authCallback = AuthCallback.builder().code(code).build();
        // 执行请求
    ...
    AuthRequest buildAuthRequest(Integer socialType, Integer userType) {
        ...
        String source = socialTypeEnum.getSource();

        // 如果是自定义的钉钉认证，直接创建 AuthDingTalkRequest
        if (SocialTypeEnum.DINGTALK.getSource().equals(source)) {
            // 获取配置
            ...
            if (client != null && Objects.equals(client.getStatus(), CommonStatusEnum.ENABLE.getStatus())) {
                ...
                config.setRedirectUri("http://localhost:48080/admin-api/system/auth/social-login-redirect");
