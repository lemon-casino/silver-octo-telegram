...
public class BpmProcessInstanceController {
    ...
    @Operation(summary = "获得流程实例的精简列表")
    @PreAuthorize("@ss.hasPermission('bpm:process-instance:query')")
    public CommonResult<PageResult<BpmProcessInstanceSimpleRespVO>> getProcessInstanceSimpleList(@Valid BpmProcessInstancePageReqVO pageReqVO) {
        // 获取流程实例列表
        PageResult<HistoricProcessInstance> pageResult = processInstanceService.getProcessInstanceList(pageReqVO);
        if (CollUtil.isEmpty(pageResult.getList())) {
            return success(PageResult.empty(pageResult.getTotal()));
        }
        ...
        // 转换返回
        return success(BpmProcessInstanceConvert.INSTANCE.buildProcessInstanceSimplePage(
                pageResult, processDefinitionMap));
    }
    ...
    @GetMapping("/stats")
    @Operation(summary = "获得流程实例统计数据", description = "统计流程中审批中、审批通过、审批不通过、已取消的数量")
    @PreAuthorize("@ss.hasPermission('bpm:process-instance:query')")
    public CommonResult<BpmProcessInstanceStatsRespVO> getProcessStats() {
