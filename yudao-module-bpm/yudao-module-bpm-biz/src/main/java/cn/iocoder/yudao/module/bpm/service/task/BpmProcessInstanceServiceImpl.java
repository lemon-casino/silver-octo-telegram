...
import cn.iocoder.yudao.module.bpm.framework.flowable.core.command.SetProcessVariablesStringCmd;
...
public class BpmProcessInstanceServiceImpl implements BpmProcessInstanceService {
    ...
    @Override
    public PageResult<HistoricProcessInstance> getProcessInstanceList(@Valid BpmProcessInstancePageReqVO pageReqVO) {
        // 构建查询条件
        HistoricProcessInstanceQuery processInstanceQuery = historyService.createHistoricProcessInstanceQuery()
                .includeProcessVariables()
                .processInstanceTenantId(FlowableUtils.getTenantId())
                .orderByProcessInstanceStartTime().desc(); // 按照流程实例的开始时间倒序
        ...
        // 查询分页数据
        List<HistoricProcessInstance> list = processInstanceQuery.listPage(PageUtils.getStart(pageReqVO), pageReqVO.getPageSize());
    ...
    public BpmApprovalDetailRespVO getApprovalDetail(Long loginUserId, BpmApprovalDetailReqVO reqVO) {
        ...
        if (reqVO.getProcessInstanceId() != null) {
            ...
            // 添加日志，输出startUserId和流程变量中的用户ID
            log.info("[getApprovalDetail] 流程实例ID: {}, 流程发起人ID(startUserId): {}, 流程变量中的发起人ID: {}", 
                    reqVO.getProcessInstanceId(), startUserId, 
    ...
    private String createProcessInstance0(Long userId, ProcessDefinition definition,
        ...
        // 设置流程变量，输出日志
        log.info("[createProcessInstance0][设置流程变量] 用户ID: {}", userId);
        ...
        log.info("[createProcessInstance0][创建流程实例成功] 流程实例ID: {}, 流程定义ID: {}, 用户ID: {}", 
                instance.getId(), instance.getProcessDefinitionId(), userId);
    ...
    public void cancelProcessInstanceByStartUser(Long userId, @Valid BpmProcessInstanceCancelReqVO cancelReqVO) {
        ...
        if (!Objects.equals(startUserId, userId)) {
        ...
        //发送消息
        messageService.sendMessageWhenProcessInstanceCancel(
                BpmProcessInstanceConvert.INSTANCE.buildProcessInstanceCancelMessage(instance));
    }
        ...
        FlowableUtils.execute(instance.getTenantId(), () -> {
            ...
            if (Objects.equals(status, BpmProcessInstanceStatusEnum.APPROVE.getStatus())) {
                messageService.sendMessageWhenProcessInstanceApprove(
                        BpmProcessInstanceConvert.INSTANCE.buildProcessInstanceApproveMessage(instance),false);
    ...
    public void updateProcessInstanceVariables(String id, Map<String, Object> variables) {
        if (CollUtil.isEmpty(variables)) {
            log.info("[updateProcessInstanceVariables][流程实例({}) 没有变量需要更新]", id);
            return;
        }
        // 1. 获取流程实例当前的变量
        ProcessInstance processInstance = getProcessInstance(id);
        if (processInstance == null) {
            log.warn("[updateProcessInstanceVariables][流程实例({}) 不存在]", id);
            return;
        }
        Map<String, Object> currentVariables = processInstance.getProcessVariables();
        
        // 2. 记录变量变更日志（用于排查问题）
        variables.forEach((key, value) -> {
            Object oldValue = currentVariables.get(key);
            log.info("[updateProcessInstanceVariables][流程实例({}) 变量({})变更：{}({})->{}({})]", 
                      id, key, oldValue, oldValue != null ? oldValue.getClass().getName() : "null", 
                      value, value != null ? value.getClass().getName() : "null");
        });
        
        // 3. 处理表单变量，确保数值类型变量以字符串形式存储
        Map<String, Object> processedVariables = new HashMap<>(variables.size());
        variables.forEach((key, value) -> {
            if (value != null) {
                // 数值类型的变量都转换为字符串存储，避免大数值被截断
                if (value instanceof Number || (value instanceof String && NumberUtil.isNumber(value.toString()))) {
                    String stringValue = value.toString();
                    processedVariables.put(key, stringValue);
                    log.info("[updateProcessInstanceVariables][流程实例({}) 数值变量({})处理：{}({})->{}(String)]", 
                          id, key, value, value.getClass().getName(), stringValue);
                } else {
                    processedVariables.put(key, value);
                }
            } else {
                processedVariables.put(key, null);
            }
        });
        
        // 4. 更新流程实例变量 - 使用自定义命令强制字符串处理
        try {
            // 方法一：使用自定义命令设置变量
            log.info("[updateProcessInstanceVariables][流程实例({}) 使用自定义命令设置变量]", id);
            try {
                Map<String, Object> cmdResults = managementService.executeCommand(new SetProcessVariablesStringCmd(id, processedVariables, false));
                log.info("[updateProcessInstanceVariables][流程实例({}) 自定义命令设置变量成功，结果: {}]", id, cmdResults);
                if (cmdResults != null && !cmdResults.isEmpty()) {
                    // 使用命令返回的结果进行验证，而不是重新查询
                    boolean allMatch = true;
                    for (Map.Entry<String, Object> entry : processedVariables.entrySet()) {
                        String key = entry.getKey();
                        Object expected = entry.getValue();
                        Object actual = cmdResults.get(key);
                        
                        boolean match = Objects.equals(expected, actual);
                        log.info("[updateProcessInstanceVariables][流程实例({}) 自定义命令变量({})验证：预期值={}({}), 实际值={}({}), 匹配={}]", 
                            id, key, expected, expected != null ? expected.getClass().getName() : "null",
                    ...
                    if (allMatch) {
                        log.info("[updateProcessInstanceVariables][流程实例({}) 自定义命令所有变量验证通过]", id);
                        return; // 如果自定义命令已成功设置并验证所有变量，直接返回
                    } else {
                        log.warn("[updateProcessInstanceVariables][流程实例({}) 自定义命令部分变量验证失败，尝试常规方式]", id);
                    }
                } else {
                    log.warn("[updateProcessInstanceVariables][流程实例({}) 自定义命令未返回结果，尝试常规方式]", id);
                }
            } catch (Exception e) {
                log.error("[updateProcessInstanceVariables][流程实例({}) 自定义命令设置变量失败: {}]", id, e.getMessage());
                // 继续使用下一种方法
            }
            
            // 方法二：使用常规方式
            log.info("[updateProcessInstanceVariables][流程实例({}) 使用常规方式设置变量]", id);
            try {
                runtimeService.setVariables(id, processedVariables);
                log.info("[updateProcessInstanceVariables][流程实例({}) 常规方式设置变量成功]", id);
            } catch (Exception e) {
                log.error("[updateProcessInstanceVariables][流程实例({}) 常规方式设置变量失败: {}]", id, e.getMessage());
                throw e; // 两种方法都失败，抛出异常
            }
            
            // 5. 验证变量更新是否成功
            ProcessInstance updatedInstance = getProcessInstance(id);
            if (updatedInstance != null) {
                Map<String, Object> updatedVariables = updatedInstance.getProcessVariables();
                boolean allUpdated = true;
                for (Map.Entry<String, Object> entry : processedVariables.entrySet()) {
                    String key = entry.getKey();
                    Object value = entry.getValue();
                    Object updatedValue = updatedVariables.get(key);
                    
                    boolean match = Objects.equals(value, updatedValue);
                    log.info("[updateProcessInstanceVariables][流程实例({}) 变量({})更新验证：预期值={}({}), 实际值={}({}), 匹配={}]", 
                          id, key, value, value != null ? value.getClass().getName() : "null",
                          updatedValue, updatedValue != null ? updatedValue.getClass().getName() : "null",
                          match);
                ...
                if (allUpdated) {
                    log.info("[updateProcessInstanceVariables][流程实例({}) 所有变量更新成功验证通过]", id);
                } else {
                    log.warn("[updateProcessInstanceVariables][流程实例({}) 部分变量更新验证失败]", id);
                }
            } else {
                log.warn("[updateProcessInstanceVariables][流程实例({}) 更新后无法获取实例，无法验证变量]", id);
            }
        } catch (Exception e) {
            log.error("[updateProcessInstanceVariables][流程实例({}) 变量更新异常]", id, e);
            throw e;
        }
    }
    ...
    @Override
    public BpmProcessInstanceStatsRespVO getProcessInstanceStats(Long userId) {
        // 1. 设置基础查询条件
        ...
        Long runningCount = historyService.createHistoricProcessInstanceQuery()
                ...
                .variableValueEquals(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_STATUS, 
                        BpmProcessInstanceStatusEnum.RUNNING.getStatus())
                .count();
        Long approveCount = historyService.createHistoricProcessInstanceQuery()
                .processInstanceTenantId(tenantId)
                .startedBy(userIdStr)
                .variableValueEquals(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_STATUS, 
                        BpmProcessInstanceStatusEnum.APPROVE.getStatus())
                .count();
        Long rejectCount = historyService.createHistoricProcessInstanceQuery()
                .processInstanceTenantId(tenantId)
                .startedBy(userIdStr)
                .variableValueEquals(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_STATUS, 
                        BpmProcessInstanceStatusEnum.REJECT.getStatus())
                .count();
        Long cancelCount = historyService.createHistoricProcessInstanceQuery()
                .processInstanceTenantId(tenantId)
                .startedBy(userIdStr)
                .variableValueEquals(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_STATUS, 
                        BpmProcessInstanceStatusEnum.CANCEL.getStatus())
