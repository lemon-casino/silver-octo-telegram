...
public class BpmModelController {
    ...
    @GetMapping("/model-manager")
    @Operation(summary = "是否拥有模型管理权限")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    public CommonResult<Boolean> getModelManagerUserIds(@RequestParam("id") String id) {
        boolean isManager = modelService.isUserProcessInstanceModelManager(getLoginUserId(), id);
        return success(isManager);
    }
