...
public class BpmTaskCandidateInvoker {
    ...
    public Set<Long> calculateUsersByTask(DelegateExecution execution) {
        ...
        return FlowableUtils.execute(execution.getTenantId(), () -> {
            ...
            removeStartUserIfSkip(userIds, flowElement, FlowableUtils.getProcessInstanceStartUserId(processInstance));
            addStartUserIfCopySelf(userIds, flowElement, FlowableUtils.getProcessInstanceStartUserId(processInstance));
            return userIds;
        });
    }
    ...
    public Set<Long> calculateUsersByActivity(BpmnModel bpmnModel, String activityId,
        ...
        addStartUserIfCopySelf(userIds, flowElement, startUserId);
        return userIds;
    }
    ...
    @VisibleForTesting
    void addStartUserIfCopySelf(Set<Long> assigneeUserIds, FlowElement flowElement, Long startUserId) {
        // 添加日志
        log.info("[addStartUserIfCopySelf][flowElement={}, copySelf={}, startUserId={}]", 
                 flowElement.getId(), BpmnModelUtils.parseCopySelf(flowElement), startUserId);
        log.info("BpmnModelUtils.parseCopySelf(flowElement)-->{}", BpmnModelUtils.parseCopySelf(flowElement));
        if (!Boolean.TRUE.equals(BpmnModelUtils.parseCopySelf(flowElement))) {
            return;
        }
        assigneeUserIds.add(startUserId);
        // 添加日志
        log.info("[addStartUserIfCopySelf][添加发起人={} 到抄送人]", startUserId);
    }
