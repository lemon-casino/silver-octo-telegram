...
public class BpmModelServiceImpl implements BpmModelService {
    ...
    @Override
    public boolean isUserProcessInstanceModelManager(Long userId, String processInstanceId) {
        if (userId == null || StrUtil.isBlank(processInstanceId)) {
            log.warn("[isUserProcessInstanceModelManager][参数无效, userId={}, processInstanceId={}]", userId, processInstanceId);
            return false;
        }
        ...
        try {
            ...
            if (processInstance == null) {
                log.warn("[isUserProcessInstanceModelManager][流程实例不存在, processInstanceId={}]", processInstanceId);
                return false;
            }
            ...
            if (StrUtil.isBlank(processDefinitionId)) {
                log.warn("[isUserProcessInstanceModelManager][流程定义ID为空, processInstanceId={}]", processInstanceId);
                return false;
            }
            ...
            if (processDefinitionInfo == null) {
                log.warn("[isUserProcessInstanceModelManager][流程定义信息不存在, processDefinitionId={}]", processDefinitionId);
                return false;
            }
            
            // 获取模型ID
            String modelId = processDefinitionInfo.getModelId();
            if (StrUtil.isBlank(modelId)) {
                log.warn("[isUserProcessInstanceModelManager][模型ID为空, processDefinitionInfo={}]", processDefinitionInfo.getId());
                return false;
            }
            
            // 获取模型并检查当前用户是否在管理员列表中
            Model model = getModel(modelId);
            if (model == null) {
                log.warn("[isUserProcessInstanceModelManager][模型不存在, modelId={}]", modelId);
                return false;
            }
            
            BpmModelMetaInfoVO metaInfo = BpmModelConvert.INSTANCE.parseMetaInfo(model);
            if (metaInfo == null) {
                log.warn("[isUserProcessInstanceModelManager][模型元信息为空, modelId={}]", modelId);
                return false;
            }
